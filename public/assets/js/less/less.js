...
module.exports = function(options, logger) {
	...
	FileManager.prototype.clearSingleFileCache = function(url) {
		
		var host = window.location.host;
		var protocol = window.location.protocol;
		var keyPrefix = protocol + '//' + host + url;
		for (var key in fileCache) {
		  
		if (key.indexOf(keyPrefix) === 0) {
			console.log(fileCache[key]);
		  delete fileCache[key];
		}
	  }
    };
	FileManager.prototype.getSingleFileCache = function(url) {
		
		var host = window.location.host;
		var protocol = window.location.protocol;
		var keyPrefix = window.basePath + url;
		for (var key in fileCache) {
	...
	FileManager.prototype.putSingleFileCache = function(url, content) {
		
		var host = window.location.host;
		var protocol = window.location.protocol;
		var keyPrefix = protocol + '//' + host + url;
		fileCache[keyPrefix] = content;
    };
