...
namespace App\MediaEmbed\Object;
...
class MediaObject implements ObjectInterface
{
     ...
     * @param array $config
     */
    public function __construct($stub, $config)
    {
        ...
        if (isset($stub['iframe-player'] )) {
            if ($config['prefer'] === 'iframe') {
                ...
                $stubarray('iframe-player') = $src;
                return true;
            }
            unset($stubarray('iframe-player'));
        }
        */
        $type = 'embed-src';
    ...
    public function getAMPEmbedCode($width, $height)
    {
        if (!empty($this->_stub['amp-code'])) {
            return $this->_buildAMP($width, $height);
        } elseif (!empty($this->_stub['iframe-player']) && $this->config['prefer'] === 'iframe') {
            return $this->_buildIframe(true);
        }
    }
    /**
    ...
    protected function _buildAMP($width, $height)
    {
        $amp = $this->_stub['amp-code'];
        for ($i = 1; $i <= count($this->_match); ++$i) {
            $amp = str_ireplace('$'.$i, $this->_match[$i - 1], $amp);
        }
        $amp = str_ireplace('$width', $width, $amp);
        $amp = str_ireplace('$height', $height, $amp);
        return $amp;
    }
    /**
     ...
     * @return string
     */
    protected function _buildIframe($amp = false)
    {
        ...
        return sprintf('<amp-iframe src="%s"%s></amp-iframe>', $source, $attributes);
    }
