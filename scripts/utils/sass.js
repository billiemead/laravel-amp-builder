export default angular.module('stgrid',['datatables','pagemanager.communication'])
.config(function($logProvider){
    $logProvider.debugEnabled(true);
})
.controller('stGridController',[ '$scope','communication','DTOptionsBuilder', 'DTColumnBuilder','$compile','dateFilter', 'safeApply', function stGridCtrl($scope,communication,DTOptionsBuilder, DTColumnBuilder,$compile,dateFilter, safeApply)
{
	$scope.columns= [];
    ...
    $scope.dtColumns = [
		/*DTColumnBuilder.newColumn(null).withTitle(titleHtml).notSortable().withOption('searchable', false)
            .renderWith(function(data, type, full, meta) {
                $scope.selected[full[$scope.primaryField]] = false;
                return '<input ng-model="selected[' + data[$scope.primaryField] + ']" ng-change="toggleOne()" type="checkbox"/>';
            }),*/
    ];
	$scope.dtColumns.push(
		DTColumnBuilder.newColumn('Button').withTitle('').notSortable().withOption('searchable', false)
            .renderWith(function(data, type, full, meta) {
				var s = '';
...
"<'row'<'col-sm-5'l><'col-sm-7'ip>>",)
	...
	if(url != undefined && url.length > 0)
	{
		$scope.dtOptions.withOption('ajax', {
			url: url,
			data: function (data) {
				if(!$scope.stFilters) return;
				for(var i in $scope.stFilters) {
					if(!$scope.stFilters[i]) continue;
					data.filters = data.filters || {};
					if(typeof $scope.stFilters[i].getMonth === 'function') {
						data.filters[i] = dateFilter($scope.stFilters[i], 'yyyy-MM-dd');
					}
					else
					data.filters[i] = $scope.stFilters[i];
				}
...
.directive('stGridColumn', ['communication','DTColumnBuilder',function(communication,DTColumnBuilder)
{
	return {
		//template:'<span></span>',
		require:'^stGrid',
						...
						switch(scope.type)
						{
							...
							case 'datetime':
								c.renderWith(function(data, type, full) {
									try
									{
										var date =  $.datepicker.parseDate( 'yy-mm-dd', data ) ;
										return  $.datepicker.formatDate( t('mm/dd/yy'), date);
									}
									catch(e)
									{
					...
					switch(scope.type)
					{
						...
						case 'datetime':
							c.renderWith(function(data, type, full) {
								...
								{
									var date =  $.datepicker.parseDate( 'yy-mm-dd', data ) ;
									return  '<a href="javascript:void(0);" ng-click="stGridRowButton.' + scope.ngClick + '(' + full[scope.primaryField] + ')">'+$.datepicker.formatDate( t('mm/dd/yy'), date)+ '</a>';
								}
								catch(e)
								{
