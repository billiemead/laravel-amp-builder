...
class History extends Module {
  ...
  change(source, dest) {
	...
	doUndo(delta)
	{
		...
		let index = getLastChangeIndex(delta);
    this.quill.setSelection(index);
	}
  clear() {
  ...
  record(changeDelta, oldDelta) {
	...
	//var state = this.undoManager._state;
	var shouldGroup = this.undoManager.isDoing();
	//if(state == this.undoManager.STATE_UNDOING || state == this.undoManager.STATE_REDOING) {
	//	shouldGroup = true;
		//return;
   // }
	this.stack.redo = [];
    ...
    if ((this.lastRecorded + this.options.delay) > timestamp && this.stack.undo.length > 0) {
     ...
     // changeDelta = delta.redo.compose(changeDelta);
	  ...
	  if(this.grouppingFlag == 0 && shouldGroup){
	...
	this.undoManager.registerUndoAction(this, this.doUndo, [undoDelta, changeDelta]);
	if(endGroup && this.grouppingFlag > 0 && shouldGroup)
	{
