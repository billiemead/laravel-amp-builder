...
(function (global, factory) {
    ...
    var Config = (function () {
        function Config() {
            this.baseUrl = window.basePath;;
            this.loginUrl = '/auth/login';
            this.signupUrl = '/auth/signup';
            this.unlinkUrl = '/auth/unlink/';
            this.tokenName = 'token';
            this.tokenPrefix = 'satellizer';
            this.refreshTokenUrl = '/auth/refresh_token';
    ...
    var Shared = (function () {
		...
		Shared.prototype.refreshToken = function ($http) {
			var deferred = this.$q.defer();
			if(this.refreshToken_doing){
				if(this.fetchRefreshUrlPromise)
					return this.fetchRefreshUrlPromise;
				deferred.resolve(0);
				return deferred.promise;
			}
			this.refreshToken_doing = true;
			
			var that = this;
            this.fetchRefreshUrlPromise = $http.get(this.SatellizerConfig.refreshTokenUrl, {
				headers: {'Authorization': 'Bearer ' + this.getToken()}
			}).then(function(response){
				that.refreshToken_doing = false;
				var headers = response.headers();
				if(headers.authorization != undefined){
					var code = headers.authorization;
					if(_.startsWith(code, 'Bearer' )){
						var token = code.substring(7);
						that.setToken({access_token:token});
						deferred.resolve(1);
						return 1;
					}
				}
		...
		Shared.prototype.redirectLogin = function()
		{
			window.is_reload = true;
			window.location.href = this.SatellizerConfig.loginUrl;
		}
    ...
    var Popup = (function () {
        ...
        Popup.prototype.open = function (url, name, popupOptions, redirectUri, dontPoll) {
			this.$interval.cancel(this.polling_timer);
        ...
        Popup.prototype.polling = function (redirectUri) {
            ...
            return this.$q(function (resolve, reject) {
                ...
                var polling = _this.polling_timer = _this.$interval(function () {
